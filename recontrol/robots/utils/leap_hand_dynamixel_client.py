...
class DynamixelClient:
    ...
    def __init__(
        ...
        Args:
            ...
            cur_scale: The scaling factor for the currents. This is
                motor-dependent. If not provided uses the default scale.
        """
        import dynamixel_sdk  # noqa: PLC0415
    ...
    def connect(self):
        ...
        if self.port_handler.openPort():
            ...
            raise OSError(
                f"Failed to open port at {self.port_name} (Check that the device is powered on and connected to your computer)."
            )
        ...
        if self.port_handler.setBaudRate(self.baudrate):
            ...
            raise OSError(
                f"Failed to set the baudrate to {self.baudrate} (Ensure that the device was configured for this baudrate)."
            )
    ...
    def set_torque_enabled(self, motor_ids: Sequence[int], enabled: bool, retries: int = -1, retry_interval: float = 0.25):
        ...
        while remaining_ids:
            ...
            if remaining_ids:
                logging.error("Could not set torque %s for IDs: %s", "enabled" if enabled else "disabled", str(remaining_ids))
            if retries == 0:
                break
    ...
    def sync_write(self, motor_ids: Sequence[int], values: Sequence[int | float], address: int, size: int):
        ...
        errored_ids = []
        for motor_id, desired_pos in zip(motor_ids, values, strict=False):
    ...
    def handle_packet_result(
        self, comm_result: int, dxl_error: int | None = None, dxl_id: int | None = None, context: str | None = None
    ):
...
class DynamixelReader:
    ...
    def __init__(self, client: DynamixelClient, motor_ids: Sequence[int], address: int, size: int):
        ...
        self.operation = self.client.dxl.GroupSyncRead(client.port_handler, client.packet_handler, address, size)
...
class DynamixelPosVelReader(DynamixelReader):
    ...
    def __init__(self, client: DynamixelClient, motor_ids: Sequence[int], pos_scale: float = 1.0, vel_scale: float = 1.0):
        super().__init__(
            client,
            motor_ids,
