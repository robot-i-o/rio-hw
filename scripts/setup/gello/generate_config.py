...
def get_joint_offsets(args: Args, port: str) -> tuple[list, tuple[float, float] | None]:
...
def update_config_with_offsets(
    ...
    port: str,
    joint_offsets: list,
    gripper_config: tuple[float, float] | None,
    args: Args,
) -> dict:
...
def main(args: Args) -> None:
    ...
    print("1. Position your GELLO in the position shown in the build documentation image.")
    ...
    try:
        ...
        with open(hardware_template_path) as f:
            hardware_template = yaml.safe_load(f)

        # Load simulation template
        with open(sim_template_path) as f:
            sim_template = yaml.safe_load(f)

        # Update configs with detected offsets
        hardware_config = update_config_with_offsets(hardware_template, port, joint_offsets, gripper_config, args)
        sim_config = update_config_with_offsets(sim_template, port, joint_offsets, gripper_config, args)

    except FileNotFoundError as e:
        print(f"Error: Template config file not found: {e}")
        print("Please ensure yam_template.yaml and yam_sim_template.yaml exist in the configs/templates/ directory.")
        sys.exit(1)
    ...
    print(f"   Hardware: python experiments/launch_yaml.py --left-config-path {hardware_output_path}")
    print(f"   Simulation: python experiments/launch_yaml.py --left-config-path {sim_output_path}")
    print()
